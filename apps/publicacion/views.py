...
class PublicacionEdicionView(UpdateView):
    ...
    def get_object(self):
        try:
            publicacion = Publicacion.objects.get(pk=self.kwargs['pk'], cliente=self.request.user.usuario)
...
class PublicacionEliminacionView(DeleteView):
    ...
    def get_object(self):
        publicacion = get_object_or_404(Publicacion, pk=self.kwargs['pk'], cliente=self.request.user.usuario)
        return publicacion
